...
Start de server vanaf de projectroot (zoals `run.bat`):
...
Beide scripts gebruiken standaard:

- `HOST=0.0.0.0`
- `PORT=8001`

Je kunt deze variabelen overschrijven in je shell.

### 2) Docker build & run

```bash
docker build -t astro-app .
docker run --rm -p 8001:8001 --env HOST=0.0.0.0 --env PORT=8001 astro-app
```

### 3) Docker Compose (aanbevolen)

Kopieer eerst `.env.example` naar `.env` en pas waarden aan waar nodig:
...
### 4) Na deploy controleren

- API health check: `GET /docs` en `POST /api/horoscoop`
- UI check: `GET /`
- Static files check: `GET /static/ljlk-logo.svg`

### 5) GitHub checklist

- Commit `Dockerfile`, `docker-compose.yml`, `.env.example`, `.gitignore`
- **Niet committen:** `.env`, virtuele omgevingen, cachebestanden
- Voeg in de GitHub-repo een korte deploy-instructie toe die verwijst naar deze sectie
