...
def test_year_pillar_is_not_lichun_day_pillar():
    """
    1988-12-13 (Gregorian) is 戊辰 (Wù Chén), Earth Dragon — not the day pillar of Lìchūn
    (which can be 丙寅 / Yín Tiger branch).
    Uses Feb 4 fallback when Lìchūn JD is unavailable.
    """
    from datetime import datetime, timezone

    epoch = datetime(2000, 1, 1, 12, 0, 0, tzinfo=timezone.utc)
    d = (datetime(1988, 12, 13, 12, 0, 0, tzinfo=timezone.utc) - epoch)
    jd_ut = 2451545.0 + d.total_seconds() / 86400.0
    idx, stem, branch = chinese.year_pillar_licheng(jd_ut, 8.0, None, None)
    assert branch == "Chén"
    assert stem == "Wù"
    assert idx == 5
